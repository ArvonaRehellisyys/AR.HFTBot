...
function GetFromYahooApiToCsv {
    [Cmdletbinding()]
    Param([string]$File, [int]$Interval)
    ...
    while($true) {
        ...
        $data = $content.query.results.ChildNodes | Select-Object AskRealTime, BidRealTime, Symbol | Add-Member -NotePropertyValue $time -NotePropertyName "Time" -PassThru

        Write-Verbose "At: $time with result count: $($data.count)"
...
GetFromYahooApiToCsv ".\Data\YahooApi.01.csv" (60*1) -Verbose